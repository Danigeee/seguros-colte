 ...
 */
export const TEMPLATE_SID_CHANNEL_MAP: Record<string, string> = {
...
const _documentChannelStore = new Map<string, string>();

// Deduplicación: guarda el timestamp del último envío de notificación por teléfono
const _lastNotificationTs = new Map<string, number>();
 ...
 */
export const saveDocumentChannel = async (clientPhone: string, templateSidOrChannel: string): Promise<void> => {
  try {
 ...
 */
export const notifySignedDocumentReceived = async (
  clientName: string,
  clientPhone: string
): Promise<void> => {
  try {
    ...
    const nameToUse = clientName || (chatHistory as any)?.client_name || 'Desconocido';
