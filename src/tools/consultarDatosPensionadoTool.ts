...
export const consultarDatosPensionadoTool = tool(
  async ({ cedula }) => {
    ...
    if (datos.sexo === 'M') datos.sexo = 'Masculino';
    else if (datos.sexo === 'F') datos.sexo = 'Femenino';

    const camposFaltantes = (
      Object.entries(datos) as [string, string | null][]
    )
  ...
  },
  {
    ...
    description:
      'Consulta la base de datos de pensionados por número de cédula. Retorna los datos disponibles y la lista de campos que faltan y deben pedirse al cliente.',
    schema: z.object({
