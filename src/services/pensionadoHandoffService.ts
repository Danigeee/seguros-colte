...
interface PensionadoHandoffPayload {
  telefono: string;
  nombre?: string;
}

export async function processPensionadoHandoff(payload: PensionadoHandoffPayload): Promise<void> {
  ...
  const updateFields: Record<string, any> = { origin: 'elevenlabs_pensionado' };
  if (nombre) updateFields.client_name = nombre;
  ...
  if (nombre) contentVariables['1'] = nombre;
