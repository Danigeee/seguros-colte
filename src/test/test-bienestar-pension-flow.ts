...
function agentePidiendoOTP(text: string): boolean {
...
// ─── EJECUTAR UN TURNO ────────────────────────────────────────────────────────
async function runTurn(userMessage: string, n: number): Promise<{ reply: string; msgCount: number }> {
  console.log(`\n${'─'.repeat(62)}`);
  ...
  const timeout = new Promise<never>((_, rej) =>
    setTimeout(() => rej(new Error('⏱️  Timeout 90s')), 90_000)
  );

  const output = await Promise.race([graph.invoke(inputs, config), timeout]) as any;

  const last = output.messages[output.messages.length - 1];
  const reply = typeof last.content === 'string' ? last.content : JSON.stringify(last.content);
  const totalMsgs: number = output.messages.length;
...
async function main() {
  ...
  try {
    ...
    const stateMsgCount: number = (finalState?.values as any)?.messages?.length ?? -1;
