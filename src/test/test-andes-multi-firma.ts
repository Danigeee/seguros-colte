...
// ─────────────────────────────────────────────
const CONFIG = {
  fondo: 'casur' as 'casur' | 'cremil' | 'fiduprevisora',
...
function fetchConTimeout(url: string, options: RequestInit, timeoutMs = 30_000): Promise<Response> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  return fetch(url, { ...options, signal: controller.signal }).finally(() => clearTimeout(timer));
}

function preguntarOTP(): Promise<string> {
...
const SOLICITUD_FIELDS: Record<string, string> = {
...
const LIBRANZA_TEXT_FIELDS: Record<string, string> = {
...
const FVC01_TEXT_FIELDS: Record<string, string> = {
...
async function cargarPdfsEnStore(): Promise<Array<{ key: string; nombre: string }>> {
  ...
  const documentos: Array<{ key: string; nombre: string }> = [];
  ...
  for (let i = 0; i < pdfFiles.length; i++) {
    ...
    if (esSOLICITUD || esLIBRANZA || esFVC01) {
      ...
      for (const [fieldName, valor] of Object.entries(textFields)) {
        try { form.getTextField(fieldName).setText(valor); } catch { /* campo ausente */ }
      }
      for (const fieldName of checkboxesOn) {
        try { form.getCheckBox(fieldName).check(); } catch { /* campo ausente */ }
      }
...
async function solicitarOTP(): Promise<void> {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(`Error al solicitar OTP: ${JSON.stringify(data)}`);
  console.log('✅  OTP solicitado:', JSON.stringify(data));
  console.log(`\n📧  Revisa el correo: ${CONFIG.correo}`);
}

async function firmarUnDocumento(key: string, nombre: string, otp: string): Promise<string> {
  const documentoBase64 = pdfBase64Store.get(key);
  if (!documentoBase64) throw new Error(`PDF no encontrado en store para key: ${key}`);
...
async function descargarYEnviarCertificados(
  solicitudes: Array<{ idSolicitud: string; nombreArchivo: string }>
): Promise<void> {
  ...
  for (const { idSolicitud, nombreArchivo } of solicitudes) {
    ...
    const signedKey = Array.from(pdfBase64Store.keys()).find(
      k => k.startsWith(`signed_pdf_${CONFIG.documento}`) && pdfBase64Store.get(k)
    );
    if (signedKey) {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status} al descargar testigo ${idSolicitud}`);
    const data = await res.json();
  ...
  if (clienteError) throw new Error(`Error Resend (cliente): ${JSON.stringify(clienteError)}`);
  ...
  if (copyError) console.warn(`   ⚠️   Error copia interna: ${JSON.stringify(copyError)}`);
  else console.log(`📧  Copia interna enviada a alejandro.b@ultimmarketing.com con ${internalAttachments.length} adjunto(s)`);
}
...
async function main() {
  ...
  try {
    ...
    const solicitudes: Array<{ idSolicitud: string; nombreArchivo: string }> = [];
