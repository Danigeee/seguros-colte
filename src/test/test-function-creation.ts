...
async function testFunctionCreation() {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            data.slice(0, 2).forEach((result: any, index: number) => {
        ...
        if (data && data.length > 1) {
            const similarities = data.map((r: any) => r.similarity).filter((s: any) => s !== null);
            const isOrdered = similarities.every((val: number, i: number, arr: number[]) => 
                i === 0 || arr[i - 1] >= val
            );
