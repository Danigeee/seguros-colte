...
import { generatePaymentLinkFlow, listarSuscripcionesPendientes, leerLinkData } from '../services/paymentsWayService.js';
import { getClientById, getClientByDocument, searchClients, splitFullName, normalizeColombianPhone } from '../functions/clientFunctions.js';
import { PRODUCTS, buildProductCharge, resolveProductKey, getProductKeys, getServiceTypes } from '../config/products.js';
const router = Router();
 ...
 */
const validateManualClient = (input) => {
 ...
 */
router.get('/seguros-colte/products', (_req, res) => {
 ...
 */
router.get('/seguros-colte/clients', async (req, res) => {
    try {
 ...
 */
router.get('/seguros-colte/advisor/subscriptions', async (req, res) => {
    try {
 ...
 */
router.post('/seguros-colte/advisor/create-link', async (req, res) => {
    try {
