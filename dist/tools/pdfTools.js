...
export const generateEstimationPdf = tool(async ({ estimation_id }) => {
    try {
      ...
      `)
        ...
        const client = estimation.client;
        if (client && client.phone) {
            try {
                await whatsappService.sendMessage(client.phone, `Hola ${client.name || 'Cliente'}, adjunto encontrarás tu cotización #${estimation.sequential_number}.`, publicUrl);
