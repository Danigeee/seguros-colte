 ...
 */
function fetchConTimeout(url, options, timeoutMs = 10_000) {
 ...
 */
export const verificarEstadoAndes = tool(async () => {
    try {
 ...
 */
export const solicitarCertificado = tool(async (input) => {
    try {
 ...
 */
export const firmarDocumento = tool(async (input) => {
    try {
        ...
        const { codigoOTP, documento, pdfKey: pdfKeyExplicita, nombreAdjunto, firmaVisible, coordenadasFirma, pagina, observaciones, tipoFirmaVis, imagenFirma } = input;
 ...
 */
export const descargarCertificado = tool(async ({ idSolicitud, correoCliente, nombreCliente, numeroIdentificacion, telefono }) => {
    try {
        ...
        if (originalPdfStoreKey)
            pdfBase64Store.delete(originalPdfStoreKey);
        if (pdfBase64 && correoCliente) {
            try {
                ...
                if (copyError)
                    console.error('[AndesTools] descargar_certificado → Error copia interna:', copyError);
                else
                    console.log('[AndesTools] descargar_certificado → Copia interna enviada a legal@ultimmarketing.com');
 ...
 */
export const descargarTodosCertificados = tool(async ({ solicitudes, correoCliente, nombreCliente, numeroIdentificacion, telefono }) => {
    try {
        ...
        for (const { idSolicitud, nombreArchivo } of solicitudes) {
            ...
            if (!res.ok)
                throw new Error(`HTTP ${res.status} para idSolicitud ${idSolicitud}`);
            const data = await res.json();
        ...
        if (copyError)
            console.error('[AndesTools] descargar_todos_certificados → Error copia interna:', copyError);
        else
            console.log(`[AndesTools] descargar_todos_certificados → Copia interna enviada a legal@ultimmarketing.com con ${internalAttachments.length} adjunto(s)`);
        return {
            success: true,
