...
function buildSolicitudFields(d) {
    return {
...
function buildLibranzaFields(d, transactionId, fechaPrimeraCuota, fechaActualLarga, fondoPagador) {
    return {
...
function buildAutorizacionFields(d, transactionId) {
    const now = new Date();
...
function buildFVC01Fields(d, fondoPagador) {
...
export const generarPdfsFondoTool = new DynamicStructuredTool({
  ...
  CRÍTICO: Guarda TODOS los elementos de esa lista — necesitarás la 'key' de CADA UNO para llamar a firmar_documento por separado. No omitas ninguno.`,
    ...
    func: async (data) => {
        try {
            const { fondoPension, nombresApellidos, primerNombre, segundoNombre, primerApellido, segundoApellido, tipoIdentificacion, numeroIdentificacion, fechaNacimiento, lugarNacimiento, sexo, direccionResidencia, ciudad, departamento, paisResidencia, telefono, email, fechaExpedicion, ingresosMensuales, numeroAfiliacion, nivelEducacion, zona, adminRecursosPublicos, esPEP, } = data;
            // Normalizar campos que pueden venir con variaciones del LLM
            const normSiNo = (v) => v.trim().toLowerCase() === 'si' || v.trim() === 'Sí' ? 'Sí' : 'No';
            ...
            for (let i = 0; i < pdfFiles.length; i++) {
                ...
                if (esSOLICITUD || esLIBRANZA) {
                    ...
                    for (const [f, v] of Object.entries(fvc01Text)) {
                        try {
                            form.getTextField(f).setText(v);
                        }
                        catch { /* campo ausente */ }
                    }
                    for (const f of fvc01Checks) {
                        try {
                            form.getCheckBox(f).check();
                        }
                        catch { /* campo ausente */ }
                    }
                    pdfFinal = await pdfDoc.save({ useObjectStreams: false });
