 ...
 */
export const create_estimation = tool(async ({ client_id, title }) => {
    try {
 ...
 */
export const search_catalog = tool(async ({ query }) => {
    try {
        const { data, error } = await supabase
            .from("items")
            .select("*")
        ...
        if (!data || data.length === 0)
            return "No se encontraron items con ese nombre.";
        // Formatear para el LLM
        const result = data.map((item) => `ID: ${item.id} | ${item.name} | Unidad: ${item.unit} | Precio Base: $${item.unit_cost}`).join("\n");
        return result;
    }
    catch (error) {
 ...
 */
export const add_item_line = tool(async ({ estimation_id, item_id, quantity, description_override }) => {
    try {
        ...
        if (itemError || !itemData)
            return "Error: Item ID no válido o no encontrado.";
 ...
 */
export const get_estimation_summary = tool(async ({ estimation_id }) => {
    try {
        // Obtener líneas
        ...
        if (!lines || lines.length === 0)
            return "La cotización está vacía (0 items).";
        let total = 0;
