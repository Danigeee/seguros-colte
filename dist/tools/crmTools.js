 ...
 */
export const lookupOrCreateClientTool = tool(async ({ name, email, phone, createIfMissing }) => {
 ...
 */
export const listClientEstimationsTool = tool(async ({ clientId }) => {
    try {
        ...
        const estimationsWithDetails = await Promise.all(estimations.map(async (est) => {
 ...
 */
export const manageQuoteContextTool = tool(async ({ clientId, action, notes, estimationId }) => {
 ...
 */
export const searchAndAddItemTool = tool(async ({ estimationId, searchQuery, quantity, unitOverride }) => {
    // 1. Search catalog
 ...
 */
export const searchClients = tool(async ({ query }) => {
    try {
